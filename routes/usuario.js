...
router.get('/', function(req, res) {
    ...
    result.forEach(item=> {delete item.senha});
    res.send(result);
});
...
router.post('/', function(req, res) {
    req.checkBody("nome", "Nome obrigatório").exists().notEmpty();
    req.checkBody("login", "Login obrigatório").exists().notEmpty();

    req.checkBody("email", "E-mail obrigatório").exists().notEmpty();
    req.checkBody("perfil", "Perfil obrigatório").exists().notEmpty();
    req.checkBody("senha", "Perfil obrigatório").exists().notEmpty();
    req.checkBody("perfil", "Perfil tem que ser entre ['ALUNO', 'ADMINISTRADOR', 'PROFESSOR']").isIn(['ALUNO', 'ADMINISTRADOR', 'PROFESSOR']);
